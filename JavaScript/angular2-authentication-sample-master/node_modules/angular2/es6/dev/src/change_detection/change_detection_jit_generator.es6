...
export class ChangeDetectorJITGenerator {
  ...
  getDirectiveFieldNames() {
    return assert.returnType((this.directiveRecords.map((d) => this.getDirective(d.directiveIndex))), assert.genericType(List, assert.type.string));
  }
  getDetectorFieldNames() {
    return assert.returnType((this.directiveRecords.filter((r) => r.isOnPushChangeDetection()).map((d) => this.getDetector(d.directiveIndex))), assert.genericType(List, assert.type.string));
  }
  getDirective(d) {
